...
/* =========================================================
   نظام التشفير الطرفي للتقارير الميدانية (E2EE - AES-256)
   - تشفير البيانات لحظة الإرسال من هاتف الفرد
   - تظل البيانات مشفرة أثناء الانتقال وفي السيرفر والسحابة
   - فك التشفير محلياً على جهاز المدير عند الاستعراض
   ========================================================= */

function getOrgEncryptionKey() {
  const me = getMe();
  if (me && me.organization && me.organization.encKey) {
    return 'CODEX_E2EE_' + me.organization.orgCode + '_' + me.organization.encKey;
  }
  const code = getOrgCode();
  return code ? 'CODEX_E2EE_' + code + '_MASTER_KEY' : 'CODEX_E2EE_SECURE_KEY_2026';
}

function encryptReportData(reportData, customKey) {
  if (typeof CryptoJS === 'undefined') {
    console.warn('CryptoJS is not loaded, sending unencrypted report');
    return reportData;
  }
  const key = customKey || getOrgEncryptionKey();
  const sensitive = {
    subject: reportData.subject || '',
    target: reportData.target || '',
    location: reportData.location || '',
    details: reportData.details || '',
    rating: reportData.rating || 'عادي',
    images: Array.isArray(reportData.images) ? reportData.images : []
  };

  try {
    const ciphertext = CryptoJS.AES.encrypt(JSON.stringify(sensitive), key).toString();
    return {
      ...reportData,
      isEncrypted: 1,
      encryptedPayload: ciphertext,
      subject: '🔒 [تقرير مشفر ومحمي E2EE]',
      target: '🔒 [بيانات مشفرة]',
      location: '🔒 [بيانات مشفرة]',
      details: '🔒 [تم تشفير محتوى وبيان هذا التقرير بتقنية التشفير الطرفي العسكري E2EE لحماية البيانات ومنع تسربها]',
      rating: 'سري',
      images: [] // الصور الحقيقية مشفرة بالكامل داخل encryptedPayload
    };
  } catch (e) {
...
function decryptReportData(report, customKey) {
  if (!report || !report.isEncrypted || !report.encryptedPayload) return report;
  if (report._isDecrypted) return report;
  if (typeof CryptoJS === 'undefined') {
    console.warn('CryptoJS not available to decrypt report');
    return report;
  }

  const key = customKey || getOrgEncryptionKey();
  try {
    const bytes = CryptoJS.AES.decrypt(report.encryptedPayload, key);
    const decryptedStr = bytes.toString(CryptoJS.enc.Utf8);
    if (decryptedStr) {
      const data = JSON.parse(decryptedStr);
      return {
        ...report,
        subject: data.subject ?? report.subject,
        target: data.target ?? report.target,
        location: data.location ?? report.location,
        details: data.details ?? report.details,
        rating: data.rating ?? report.rating,
        images: Array.isArray(data.images) ? data.images : (report.images || []),
        _isDecrypted: true,
        _wasEncrypted: true
      };
    }
  } catch (e) {
    console.warn('Decryption error for report:', report.reportNumber, e);
  }
  return report;
}
